...
function getSumBetweenNumbers(n1, n2) {
  const range = Array.from({ length: (n2 - n1 + 1) }, (_, i) => i + n1);
...
function isInsideCircle(circle, point) {
  ...
  if (circle.radius > radius) return true;
  return false;
}
...
function isBracketsBalanced(str) {
  ...
  [...str]
    .forEach((item) => {
      if (brecO.some((br) => br === item)) {
        brackets.push(item);
      } else if (brackets.some((br) => brecC.indexOf(item) === brecO.indexOf(br))) {
        brackets.pop();
      } else {
        brackets.push(item);
      }
...
function getCommonDirectoryPath(pathes) {
  ...
  const searchMatch = ((item) => {
    myReg = new RegExp(`,${item}`, 'g');
    return (str.match(myReg) || []).length;
  });

  if (searchMatch('/') < length) return '';

  pathFirst.split('/').slice(1).forEach((item) => {
    itemResult = `${result}${item}/`;
    lengthMatch = searchMatch(itemResult);
    if (lengthMatch === length || (lengthMatch > length && item === '/')) {
      result = itemResult;
    }
  });
  return result;
...
function evaluateTicTacToePosition(position) {
  ...
  function win(item) {
    let i = 0;
    while (i < 3) {
      if (`${p[i][0]}${p[i][1]}${p[i][2]}` === item.repeat(3)) return item;
      i += 1;
    }

    i = 0;
    while (i < 3) {
      if (`${p[0][i]}${p[1][i]}${p[2][i]}` === item.repeat(3)) return item;
      i += 1;
    }

    if (`${p[0][0]}${p[1][1]}${p[2][2]}` === item.repeat(3)) return item;
    if (`${p[0][2]}${p[1][1]}${p[2][0]}` === item.repeat(3)) return item;
    return null;
  }
